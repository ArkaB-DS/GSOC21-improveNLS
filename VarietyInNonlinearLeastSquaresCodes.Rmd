...
In @Marquardt63, it is shown that the use of the diagonal elements of $Jt J$
...
?? note that we form JTJ in nlfb, which is extra work. Then sqrt(lambda),
so may want to think more about this.
...
R object class) by building an `nlsModel` object, which we will lable `m`. This 
...
The particular QR calculations
