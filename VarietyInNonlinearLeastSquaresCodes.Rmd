...
$$ (J^t J + \lambda 1_n)  \delta  = A \delta =  - J^t r $$
...
- Linking of data to the problem. In particular, R allows **subsetting**
  ...
  hidden from the user or programmer, so that it could be dangerous to
...
The `nls()` function has an argument `subset` and a test (??need to reference or
include this??) showed this works fine. However, nowhere have we found a published
example of the use of the `subset` argument. Worse, it clearly must be applied to
...
HYPOTHESIS: nls() has `subset` and this is automatically applied to the functions
it calls. This includes `incr` and `setPars()` which contain functions to compute
the Gauss-Newtong. However, this does not mesh well with the approach taken by
`nlsr`. We note that `minpack.lm::nlsLM` also has a `subset` argument.
...
of checking, which we might leave out of nlsj() in some cases.
...
though the actualy performance of the computations may well be influenced by such factors.
...
- Eigendecompositions of the SSCP: the eigenvalues of real, symmetric matrices
...
where $Q$ is $m x n$ orthogonal so 

$$ Q^t Q = I_n $$

and $R$ is upper triangular and $n x n$. Some variants have $Q$ an $m x m$ orthogonal
...
a full $m x m$ matrix, limited to $m x n$ or even truncated to $m x r$ where 
$r$ is the rank of $K$.
