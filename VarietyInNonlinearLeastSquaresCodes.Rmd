...
to adjust its value. For example, if $SS(x) = r' r = f(x)$ is the function to 
...
e.g., 0.4) and
increase factor (e.g., 10) for $\lambda$, but there are a number of 
...
An approximate solution via the normal equations can be found by adding a small diagonal matrix to the sum of squares and
cross products `AxA`.
...
The minimum length least squares solution is found using the Moore-Penrose inverse of `Ax`
?? need some refs?? which can be computed from the Singular Value Decomposition.
