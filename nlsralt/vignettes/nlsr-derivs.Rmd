...
require(nlsralt)
dnlsr <- nlsralt::nlsDeriv(~ sin(x+y), "x")
print(dnlsr)
class(dnlsr)
detach("package:nlsralt", unload=TRUE)
...
`nlsralt::codeDeriv` are intended to output an expression to compute a derivative. 
...
- the base tool `D` and `nlsralt::nlsDeriv` generate expressions, but `D` requires an
...
if ("nlsralt" %in% loadedNamespaces() ){detach("package:nlsralt", unload=TRUE)}
...
```{r chunk14}
require(nlsralt)
dlogx <- nlsralt::nlsDeriv(~ log(x), "x")
str(dlogx)
print(dlogx)
```
...
```{r chunk15}
require(nlsralt)
dlogxs <- nlsralt::nlsDeriv(expression(log(x)), "x", do_substitute=FALSE)
str(dlogxs)
print(dlogxs)
cat(as.character(dlogxs), "\n")
fne <- expression(log(x))
dlogxe <- nlsralt::nlsDeriv(fne, "x", do_substitute=FALSE)
str(dlogxe)
print(dlogxe)
...
```{r chunk21}
try(nlsralt::nlsDeriv(zzz, "y[3]"))
try(nlsralt::nlsDeriv(y3*r1+r2,"y3"))
try(nlsralt::nlsDeriv(y[3]*r1+r2,"y[3]"))
```
